...
! write(idat,"(' Generated by Molden2AIM, Version ',a5,' (',a10,')')")ver,dt

 call fdate(cdate)
 call trulen(cdate,L1,L2,length)
 write(idat,"(' Generated by Molden2AIM, Version ',a5,' (',a10,')',11x,'Time: ',a24)")ver,dt,cdate(L1:L2)
...
Subroutine filename(ifcomm,imod,fnam,fmdn,fwfn,fwfx,fnbo,fmod)
 ...
 if(lend == 0)then                 ! use default file name
   ...
   lend=6
   fmod(1)(1:6)='MOLDEN'
 else if ( fmod(1)(1:2) == '~/' .or. fmod(1)(1:2) == '~\' ) then
   call getenv( 'HOME', fnam )
   if(len_trim( fnam ) < 1) then    ! Windows. The drive letter is saved in HOMEDRIVE
     call getenv( 'HOMEPATH', fnam )
     if(fnam(2:2) /= ":") fnam = "C:" // fnam( 1 : len_trim(fnam) )
   end if
   fmod(1) = fnam( 1 : len_trim(fnam) ) // fmod(1)( 2 : len_trim( fmod(1) ) )
   lstr=nonspace(fmod(1)(:))
   lend=len_trim(fmod(1)(:))
 end if
...
!<<< Comment out this part if hostnm is not supported by your Fortran compiler.
   call getenv( 'COMPUTERNAME', prefix(1) )    ! it works only in Windows
   if(len_trim( prefix(1) ) < 1) call hostnm( prefix(1) )
!>>>
...
!<<< Comment out this part if getcwd or getenv is not well supported by your Fortran compiler.
   ...
   if(ifdos) then
      !write(*,"(/,'DOS/Windows',/)")
     if (fname(2:3) == ':' // bslash .or. fname(2:3) == ':/') then    ! C:\ in Windows and C:/ in MSYS
       ! fulnam(1) = fname
     else if ( fname(1:2) == '~/' .or. fname(1:2) == '~\' ) then
       call getenv( 'HOME', prefix(1) )    ! it works only in MSYS & Cygwin
       if(len_trim( prefix(1) ) < 1) then    ! Windows. The drive letter is saved in HOMEDRIVE
         call getenv( 'HOMEPATH', prefix(1) )
         if(prefix(1)(2:2) /= ":") prefix(1) = "C:" // prefix(1)( 1 : len_trim(prefix(1)) )
       end if
...
!>>>
   ...
   if(mode2 == 1) then
     write(iout,"(1x,a,', ',a)") trim(hstnam(1)),trim(fulnam(1))
   else
     write(iout,"(1x,'Host name: ',a,/,1x,'File name: ',a)") trim(hstnam(1)),trim(fulnam(1))
   end if
