...
2. The `[Nval]` data block (suggested by [Multiwfn](http://multiwfn.codeplex.com/)) may be used for ECP basis sets.
3. Ghost atom in the MOLDEN file has been supported, which may be specified by a prefix `bq-`, a prefix `ghost-`, a suffix `-bq`, a suffix `-ghost`, or `atomic_number` = 0.
...
Starting from Version 5.1.0, the `[Nval]` block suggested by [Multiwfn](http://multiwfn.codeplex.com/) may also be used.
